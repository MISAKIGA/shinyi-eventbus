...
package com.shinyi.eventbus.util;
...
public class JsonUtils {
     ...
     * @return an object of type T from the string classOfT
     */
    public static <T,T2> T parseObject(String json, Class<T> clazz, Class<T2> paramClazz) {
        if (StrUtil.isEmpty(json)) {
            return null;
        }

        try {
            JavaType javaType = OBJECT_MAPPER.getTypeFactory().constructParametricType(clazz, paramClazz);
